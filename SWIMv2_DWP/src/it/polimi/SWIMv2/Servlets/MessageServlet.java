	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(messageBean.validReceiver(request.getParameter("receiverEmail"))){
				...
				if(delivered){
					request.getSession().setAttribute("alert", "Invio riuscito!");
					getServletConfig().getServletContext().getRequestDispatcher("/messagebox.jsp").forward(request, response);
				}
				
				else{
					request.getSession().setAttribute("alert", "Invio fallito: better luck next time!");
					getServletConfig().getServletContext().getRequestDispatcher("/sendmessage.jsp").forward(request, response);
				}
			}
			else{
				request.getSession().setAttribute("alert", "Invio fallito: better luck next time!");
